...
# def generate_knownhosts_json(containers, name_to_ip, container_directory, tcp_port, udp_port, filename):
#   knownhosts_dict = dict()

#   for name, info in containers.items():
#     ports = info.get('number_of_ports', 1)

#     if ports <= 0:
#       raise SystemExit("ERROR: Each host must be assigned 1 or more ports")

#     ip_address = name_to_ip[name]
#     knownhosts_dict['hosts'] = dict()
#     knownhosts_dict['hosts'][name] = dict()
#     knownhosts_dict['hosts'][name]['tcp_start_port'] = tcp_port
#     knownhosts_dict['hosts'][name]['tcp_end_port']   = tcp_port + ports - 1
#     knownhosts_dict['hosts'][name]['udp_start_port'] = udp_port
#     knownhosts_dict['hosts'][name]['udp_end_port']   = udp_port + ports - 1
#     knownhosts_dict['hosts'][name]['ip_address']     = ip_address
...
def generate_knownhosts_txt(containers, container_directory, starting_port, filename):
  with open(filename, 'w') as outfile:
    for name, info in containers.items():
      ...
      if ports > 1:
        raise SystemExit("ERROR: Multiple ports will be supported in a future release.")
...
def main():
  ...
  # Host number for a container's ip, starting at 2.
  host = 2
  name_to_ip = dict()
  for container_name, container_info in network.items():
    ...
    # Connect the container to the network with a unique ip address.
    ip = f'{network_num}.{user_num}.{subnet}.{host}'
    docker_network.connect(container, ipv4_address=ip, aliases=[container_name,])
    name_to_ip[container_name] = ip
    host += 1

  # Generating knownhosts file:
  for container_name, container_info in network.items():
    full_container_name = f'{username}_{container_name}'
    container_dir = os.path.join(working_directory, container_name)
    # Knownhosts json will be supported in a future release
    # generate_knownhosts(network, 
    #                     name_to_ip, 
    #                     container_dir, 
    #                     9000, 
    #                     10000,  
